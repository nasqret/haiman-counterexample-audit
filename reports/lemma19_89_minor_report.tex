...
degree-89 predecessor modules.  It also now records the more precise
minimality-relevant list: the 15 Schur modules \(V=S_\lambda W\) for which
\(V\otimes S_\mu W\) can contribute to the target \(S_\nu W\), at the
Littlewood--Richardson/tensor-product level.  This is a serious gap in the
written proof, but it is not by itself a proof that the claimed degree-90
generator is nonminimal, nor a proof that Lee's theorem is false.
\end{abstract}
...
\begin{enumerate}
  \item the 15 Littlewood--Richardson/tensor candidates are the only
  Schur-highest weights \(\lambda\) presently certified for a degree-89
  module \(V=S_\lambda W\) whose product with \(S_\mu W\) can contain
  \(S_\nu W\);
  \item the displayed \(89\times89\) minor is outside those 15 modules, so it
  refutes the paper's broad wording, but it is not a candidate \(V\) that can
  prove nonminimality of the target \(S_\nu W\).
\end{enumerate}
...
\subsection{Answer to the requested \texorpdfstring{\(V\)}{V}-production step}

At the level currently certified, the possible degree-89 Schur modules are
exactly
\[
  V_\lambda=S_\lambda W
\]
for the 15 \(\lambda\)'s in the table above.  For each one the audit checks
\[
  c^\nu_{\lambda,\mu}=1
\]
and supplies an explicit chain of 88 nonzero Littlewood--Richardson
coefficients proving occurrence in the tensor power
\[
  S_\lambda W \subset (S_\mu W)^{\otimes 89}.
\]
Consequently each listed \(V_\lambda\) is a necessary tensor-level candidate
for a degree-89 source whose multiplication by degree-one variables can
contribute to \(S_\nu W\) in degree 90.

This is precisely what the file
\begin{center}
  \cert{results/certificates/lemma19_minimality_relevant_modules.json}
\end{center}
records.  It deliberately labels two further tasks as open:
\begin{enumerate}
  \item construct the actual copies of these modules inside
  \(\Sym^{89}(S_\mu W)\), not merely inside the tensor power;
  \item test the corresponding embedded copies against \(J_8\), or compute
  their multiplication image in the \(S_\nu W\)-isotypic component of degree
  90.
\end{enumerate}

Thus the audit can now produce the candidate highest weights for \(V\), but it
has not yet produced the embedded submodules whose \(J_8\)-membership would
settle Lee's minimality assertion.

\subsection{Current status of the symmetric-power multiplicity computation}

The next representation-theoretic gate is the multiplicity
\[
  [S_\lambda W:\Sym^{89}(S_\mu W)]
\]
for the 15 candidates above.  The current repository reduces these numbers to
explicit GL5 plethysm coefficients recorded in
\begin{center}
  \cert{results/certificates/lemma19_symmetric_multiplicity_targets.json}.
\end{center}
The accepted certificate is now
\begin{center}
  \cert{results/certificates/lemma19_symmetric_multiplicities.json}.
\end{center}
It proves nonzero residues modulo \(1000000007\) for all 15 degree-89
candidates and also for the degree-90 target.

The Sage route
\begin{center}
  \cert{artifacts/sage/compute_symmetric_multiplicities.sage}
\end{center}
...
\]
to turn the GL5 plethysm coefficients into ordinary
Littlewood--Richardson products.  This is a theorem-level reduction, but it is
currently only a secondary implementation route: the direct summation did not
return the first target within 90 seconds on the local machine.

The Rust route
\begin{center}
  \cert{artifacts/rust/src/bin/plethysm_multiplicity.rs}
\end{center}
uses a modular semistandard-column dynamic program and then extracts Schur
multiplicities with the Weyl character formula
\[
  m_\alpha=
  \sum_{\sigma\in S_5}\operatorname{sgn}(\sigma)\,
  M(\alpha+\rho-\sigma\rho),\qquad \rho=(4,3,2,1,0).
\]
After changing residue storage from 64-bit to 32-bit arithmetic, the dense
weight-DP completed locally in about 389 seconds and wrote the certificate
above.  Since every recorded residue is nonzero modulo \(1000000007\), every
corresponding integer multiplicity is nonzero.

Therefore the symmetric-power occurrence question is closed at the
nonvanishing level.  The remaining tasks are to construct the relevant
highest-weight vectors and test their \(J_8\)-membership.
...
\begin{enumerate}
  ...
  \item The only currently certified degree-89 Schur-highest weights that can
  feed \(S_\nu W\) through \(S_\lambda W\otimes S_\mu W\) are the 15 listed
  above; this is recorded in
  \cert{lemma19_minimality_relevant_modules.json}.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item This does not construct the actual embedded \(S_\lambda W\)-copies
  inside \(\Sym^{89}(S_\mu W)\).
  \item This does not perform Lee's unpublished test that none of those
  embedded copies lies in \(J_8\).
\end{enumerate}
...
\level{Computation/certificate} \\
\midrule
Minimality-relevant \(V=S_\lambda W\) candidates &
\cert{lemma19_minimality_relevant_modules.json}; same 15 \(\lambda\)'s, with
\(c^\nu_{\lambda,\mu}=1\) and tensor-chain pointers &
\level{Computation/certificate, tensor level only} \\
\midrule
Weight exclusion from the 15 modules &
Highest-weight theory: weights of \(S_\lambda W\) are dominated by \(\lambda\) &
\level{Theorem} \\
\midrule
Copies inside \(\Sym^{89}(S_\mu W)\) &
Requires plethysm/symmetrization multiplicity-space data &
\level{Open} \\
\midrule
...
\begin{itemize}
  ...
  \item \cert{results/certificates/lemma19_minimality_relevant_modules.json};
