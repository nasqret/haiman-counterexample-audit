...
\end{center}
Two implementation routes are now present, but neither is an accepted
certificate yet.
...
Littlewood--Richardson products.  This is a theorem-level reduction, but the
current direct summation did not return the first target within 90 seconds on
the local machine.
...
multiplicities with the Weyl character formula.  A nonzero residue modulo a
prime would be a rigorous nonzero certificate, but the current dense-state
implementation reached about \(7\cdot10^5\) states at column 11 of 29 and was
interrupted as too slow/high-memory.

Therefore the multiplicity values themselves remain open.  The next technical
task is algorithmic pruning, not a change in the mathematical target.
