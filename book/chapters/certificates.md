...
- `lemma19_symmetric_multiplicities.json`: modular Rust certificate that all 15 degree-89 candidates and the degree-90 target occur with nonzero symmetric-power multiplicity.
...
The symmetric-multiplicity target artifact reduces the next representation problem to coefficients of \(S_\alpha V\) in \(S_\pi(\operatorname{Sym}^2 V)\) for \(\dim V=5\).

The accepted symmetric-multiplicity artifact is now
`lemma19_symmetric_multiplicities.json`. It was generated by
`artifacts/rust/src/bin/plethysm_multiplicity.rs`, which computes the relevant
weight multiplicities by semistandard-column dynamic programming and extracts
Schur multiplicities using the Weyl character formula.  Every listed residue
is nonzero modulo \(1000000007\), so every corresponding integer multiplicity
is nonzero.

This proves occurrence in the required symmetric powers. It does not construct
explicit highest-weight vectors in the determinant/cofactor span and does not
test \(J_8\)-membership.
