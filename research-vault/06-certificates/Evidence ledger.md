...
| Symmetric-power multiplicity targets | reduced | `lemma19_symmetric_multiplicity_targets.json` | GL5 plethysm coefficients listed |
| Symmetric-power multiplicity nonvanishing | certified mod prime | `lemma19_symmetric_multiplicities.json` + Rust DP tests | all 15 degree-89 candidates and degree-90 target nonzero mod 1000000007 |
...
| Copies of those candidates inside \(\operatorname{Sym}^{89}(S_\mu W)\) | occurrence certified, vectors pending | plethysm multiplicity certificate | nonzero multiplicity; explicit embeddings not constructed |
